-- Patch: reorder cliques/clique_members creation to fix forward reference

-- Drop the partially-applied policy if it exists
DROP POLICY IF EXISTS "cliques: read if member or owner" ON cliques;

-- Create clique_members BEFORE the cliques RLS policy that references it
...
-- Now safe to create the cliques policy that references clique_members
...
CREATE POLICY IF NOT EXISTS "clique_members: read if member or owner"
...
CREATE POLICY IF NOT EXISTS "conn_req: read own"
  ON connection_requests FOR SELECT
  USING (from_user_id = auth.uid() OR to_user_id = auth.uid());

CREATE POLICY IF NOT EXISTS "conn_req: insert from self"
  ON connection_requests FOR INSERT
  WITH CHECK (from_user_id = auth.uid());

CREATE POLICY IF NOT EXISTS "conn_req: recipient updates status"
...
CREATE POLICY IF NOT EXISTS "moment_shares: read own"
  ON moment_shares FOR SELECT
  USING (from_user_id = auth.uid() OR to_user_id = auth.uid());

CREATE POLICY IF NOT EXISTS "moment_shares: insert from self"
...
CREATE POLICY IF NOT EXISTS "calendar_events: own only"
...
CREATE POLICY IF NOT EXISTS "conversations: read if participant"
...
CREATE POLICY IF NOT EXISTS "conversations: insert"
  ON conversations FOR INSERT WITH CHECK (true);

CREATE POLICY IF NOT EXISTS "conv_participants: read own"
  ON conversation_participants FOR SELECT
  USING (user_id = auth.uid());

CREATE POLICY IF NOT EXISTS "conv_participants: insert"
  ON conversation_participants FOR INSERT WITH CHECK (true);

CREATE POLICY IF NOT EXISTS "messages: read if participant"
...
CREATE POLICY IF NOT EXISTS "messages: insert own"
...
-- Alter spark_library pk to TEXT (for slug IDs)
ALTER TABLE spark_library ALTER COLUMN id TYPE TEXT;

-- Spark seed (upsert — safe to re-run)
