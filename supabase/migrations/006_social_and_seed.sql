-- ============================================================
-- Migration 006: Social tables, messaging, and spark seed data
-- ============================================================
...
CREATE INDEX IF NOT EXISTS conn_req_to_idx ON connection_requests (to_user_id, status);
CREATE INDEX IF NOT EXISTS conn_req_from_idx ON connection_requests (from_user_id, status);

ALTER TABLE connection_requests ENABLE ROW LEVEL SECURITY;

CREATE POLICY "conn_req: read own"
  ON connection_requests FOR SELECT
  USING (from_user_id = auth.uid() OR to_user_id = auth.uid());

CREATE POLICY "conn_req: insert from self"
  ON connection_requests FOR INSERT
  WITH CHECK (from_user_id = auth.uid());

CREATE POLICY "conn_req: recipient updates status"
...
CREATE INDEX IF NOT EXISTS moment_shares_to_idx ON moment_shares (to_user_id, created_at DESC);

ALTER TABLE moment_shares ENABLE ROW LEVEL SECURITY;

CREATE POLICY "moment_shares: read own"
  ON moment_shares FOR SELECT
  USING (from_user_id = auth.uid() OR to_user_id = auth.uid());

CREATE POLICY "moment_shares: insert from self"
  ON moment_shares FOR INSERT
  WITH CHECK (from_user_id = auth.uid());
...
CREATE POLICY "calendar_events: own only"
...
CREATE INDEX IF NOT EXISTS cliques_owner_idx ON cliques (owner_id);

ALTER TABLE cliques ENABLE ROW LEVEL SECURITY;

CREATE POLICY "cliques: read if member or owner"
  ON cliques FOR SELECT
  USING (
    owner_id = auth.uid()
    OR EXISTS (SELECT 1 FROM clique_members WHERE clique_id = id AND user_id = auth.uid())
  );

CREATE POLICY "cliques: insert own"
  ON cliques FOR INSERT
  WITH CHECK (owner_id = auth.uid());

CREATE POLICY "cliques: update own"
  ON cliques FOR UPDATE
  USING (owner_id = auth.uid());

CREATE POLICY "cliques: delete own"
  ON cliques FOR DELETE
  USING (owner_id = auth.uid());
...
CREATE POLICY "clique_members: read if member or owner"
  ON clique_members FOR SELECT
  USING (
    user_id = auth.uid()
    OR EXISTS (SELECT 1 FROM cliques WHERE id = clique_id AND owner_id = auth.uid())
  );

CREATE POLICY "clique_members: owner inserts"
  ON clique_members FOR INSERT
  WITH CHECK (EXISTS (SELECT 1 FROM cliques WHERE id = clique_id AND owner_id = auth.uid()));

CREATE POLICY "clique_members: owner deletes"
  ON clique_members FOR DELETE
  USING (EXISTS (SELECT 1 FROM cliques WHERE id = clique_id AND owner_id = auth.uid()));
...
CREATE POLICY "conversations: read if participant"
...
CREATE INDEX IF NOT EXISTS conv_participants_user ON conversation_participants (user_id);
...
CREATE INDEX IF NOT EXISTS messages_conv_sent ON messages (conversation_id, sent_at DESC);

ALTER TABLE messages ENABLE ROW LEVEL SECURITY;

CREATE POLICY "messages: read if participant"
...
-- ── SPARK LIBRARY SEED DATA ──────────────────────────────────
-- spark_library.id in migration 005 is UUID; we insert with gen_random_uuid()
-- but the app references sparks by a TEXT slug id. We alter to use TEXT pk.
